...
import { FetchType } from "./constants";
...
const defaultOptions: Partial<GraphQLProviderOptions> = {
  ...
  introspection: {
    operationNames: {
      [GET_LIST]: (resource) => `all${pluralize(resource.name) as string}`,
      [GET_ONE]: (resource) => `${resource.name}`,
      [GET_MANY]: (resource) => `all${pluralize(resource.name) as string}`,
      [GET_MANY_REFERENCE]: (resource) => `all${pluralize(resource.name) as string}`,
...
export type GraphQLProviderOptions<OtherOptions = any> = {
  ...
  readonly introspection?: IntrospectionOptions
  readonly resolveIntrospection?: (
...
export default async <Options extends Record<string, any> = any>(
  ...
  if (override && process.env.NODE_ENV === "production") {
    console.warn(
            // eslint-disable-line
      "The override option is deprecated."
    );
  }
