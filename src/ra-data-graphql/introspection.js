...
export var filterTypesByIncludeExclude = function (_a) {
    var include = _a.include, exclude = _a.exclude;
    if (Array.isArray(include)) {
        return function (type) { return include.includes(type.name); };
    }
    if (typeof include === 'function') {
        return function (type) { return include(type); };
    }
    if (Array.isArray(exclude)) {
        return function (type) { return !exclude.includes(type.name); };
    }
    if (typeof exclude === 'function') {
        return function (type) { return !exclude(type); };
    }
            ...
            case 3:
                ...
                queries = schema.types.reduce(function (acc, type) {
                    if (type.name !== schema.queryType.name &&
                        ...
                        return acc;
                    var _a = type.fields, fields = _a === void 0 ? [] : _a;
                    return __spreadArrays(acc, fields);
                }, []);
